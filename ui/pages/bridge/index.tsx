...
import { resetBackgroundSwapsState, ignoreTokens } from '../../store/actions';
...
const CrossChainSwap = () => {
  ...
  useGasFeeEstimates(undefined);

  const { balance: ethBalance, address: selectedAccountAddress } =
    selectedAccount;

  const approveTxData =
    approveTxId && txList.find(({ id }: any) => approveTxId === id);
  const tradeTxData =
    tradeTxId && txList.find(({ id }: any) => tradeTxId === id);
  const approveError =
